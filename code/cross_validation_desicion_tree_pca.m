...
% This function uses PCA algorithm to reduce the Dimensionality of original
...
function [tree_confusion_matrix, tree_eval_matrix, cross_validation_matrix] = cross_validation_desicion_tree_pca(example, target)
    ...
    % Doing pca before spliting dataset into training set and testing set.
    % The reason why we do that is addressed in the report.
    [example, ~] = pre_process_pca(example);
    for i = 1 : k
        ...
        for j = 1 : emotion
            binary_target = (train_target == j);
            trees{j} = decision_tree_learning(train_input, binary_target);
        end
